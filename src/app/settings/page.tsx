...
const Input = ({ className, isSaving, onSave, isApiKey, ...restProps }: InputProps) => {
  const [showApiKey, setShowApiKey] = useState(false);
  
  // If it's an API key and not showing, convert to dots
  const displayValue = (value: string | number | readonly string[] | undefined) => {
    if (isApiKey && !showApiKey && typeof value === 'string' && value) {
  ...
  return (
    <div className="relative">
      <input
        {...restProps}
        type={isApiKey && !showApiKey ? "password" : restProps.type}
        value={displayValue(restProps.value)}
        className={cn(
          ...
          (isSaving || isApiKey) && 'pr-10',
          className,
        )}
        onBlur={(e) => onSave?.(e.target.value)}
      />
      {isApiKey && (
        <div className="absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer" onClick={() => setShowApiKey(!showApiKey)}>
          <svg 
      ...
      {isSaving && (
        <div className={`absolute ${isApiKey ? 'right-10' : 'right-3'} top-1/2 -translate-y-1/2`}>
          <Loader2
            size={16}
...
const Page = () => {
  ...
  const [selectedSearchEngine, setSelectedSearchEngine] = useState<string>('searxng');
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      ...
      setSelectedSearchEngine(data.selectedSearchEngine || 'searxng');
        ...
        config && (
          <div className="flex flex-col space-y-6 pb-28 lg:pb-8">
                ...
                <div className="flex flex-col space-y-1">
                  ...
                  <Select
                    ...
                    onChange={(e) => {
                      ...
                      saveConfig('selectedSearchEngine', value);
                    }}
                    options={[
                  ...
                  <div className="flex flex-col space-y-1">
                    ...
                    <div className="flex flex-col">
                      <Input
                        ...
                        onSave={(value) =>
                          saveConfig('braveApiKey', value)
                        }
                      />
                      <p className="text-xs text-black/60 dark:text-white/60 mt-1.5">
                        Configure your Brave API key in the .env file for better security.
                      </p>
                    </div>
                  </div>
                )}
                
                {selectedSearchEngine === 'serper' && (
                  <div className="flex flex-col space-y-1">
                    <p className="text-black/70 dark:text-white/70 text-sm">
                      Serper API Key
                    </p>
                    <div className="flex flex-col">
                      <Input
                        ...
                        isSaving={savingStates['serperApiKey']}
                        isApiKey={true}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                          setConfig((prev) => ({
                            ...prev!,
                        ...
                        onSave={(value) =>
                          saveConfig('serperApiKey', value)
                        }
                      />
                      <p className="text-xs text-black/60 dark:text-white/60 mt-1.5">
                        Configure your Serper API key in the .env file for better security.
                      </p>
                    </div>
                  </div>
                )}
                
                {selectedSearchEngine === 'searxng' && (
                  <div className="flex flex-col space-y-1">
                    <p className="text-black/70 dark:text-white/70 text-sm">
                      SearxNG URL
                    </p>
                    ...
                    />
                    <p className="text-xs text-black/60 dark:text-white/60 mt-0.5">
                      Update the SEARXNG_API_URL in your .env file to change this setting.
                    </p>
                  </div>
                )}
              </div>
            ...
            <SettingsSection title="API Endpoints">
              <div className="flex flex-col space-y-4">
                <div className="p-4 bg-blue-500/10 text-blue-600 dark:text-blue-400 rounded-lg mb-2">
                  <p className="text-sm">
                    <strong>Note:</strong> API keys (OpenAI, GROQ, Anthropic, Gemini, Deepseek) should be configured in the <code>.env</code> file for security reasons.
                  </p>
                </div>
